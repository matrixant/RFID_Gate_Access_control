...
unsigned char Rx_FIFO_ReadChar()
{
	...
	temp=Rx_FIFO[Rx_FIFO_IndexR];				//将读指针位置的FIFO数据赋给指针所指变量
...
char Tx_FIFO_WriteChar(unsigned char data)
{
    ...
    __disable_interrupt();       	            			//操作FIFO前一定要关总中断
	//-----“全新”一次发送数据必须手动触发Tx中断-----
	if((Tx_FIFO_DataNum==0) &&(!(UCA0STATW & UCBUSY)))		//判断是否为一次“全新”发送
        UCA0IFG |= UCTXIFG;									// 手动触发一次
...
unsigned char Tx_FIFO_ReadChar()
{
    ...
    tmp=Tx_FIFO[Tx_FIFO_IndexR];                          	//将读指针位置的FIFO数据赋给指针所指变量
